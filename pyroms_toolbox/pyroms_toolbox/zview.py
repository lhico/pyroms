...
def zview(var, tindex, depth, grid, filename=None, \
    ...
    if proj == 'stere':
        projection = ccrs.Stereographic(central_latitude=90, central_longitude=180)
        fig, ax_map = plt.subplots(subplot_kw={'projection': projection})
        ax_map.set_extent([-210, -50, 40, 50], crs=ccrs.PlateCarree())
        x, y = lon, lat
    ...
    if fill:
        if proj is not None:
            cf = ax_map.contourf(x, y, zslice, vc, cmap=pal, norm=pal_norm, transform=ccrs.PlateCarree())
        else:
            cf = plt.contourf(lon, lat, zslice, vc, cmap=pal, norm=pal_norm)
    else:
        if proj is not None:
            cf = ax_map.pcolormesh(x, y, zslice, cmap=pal, norm=pal_norm, transform=ccrs.PlateCarree())
        else:
    ...
    if contour:
        if not fill:
            ...
            if proj is not None:
                ax_map.contour(x, y, zslice, vc[::d], colors='k', linewidths=0.5, linestyles='solid', transform=ccrs.PlateCarree())
            else:
