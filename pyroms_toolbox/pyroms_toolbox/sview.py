...
def sview(var, tindex, sindex, grid, filename=None, \
    ...
    if proj is not None:
        projection = ccrs.Mercator() if proj == 'merc' else ccrs.PlateCarree()
    ...
    if fill:
        if proj is not None:
            cf = ax_map.contourf(x, y, sslice, vc, cmap=pal, norm=pal_norm, transform=ccrs.PlateCarree())
        else:
            cf = plt.contourf(lon, lat, sslice, vc, cmap=pal, norm=pal_norm)
    else:
        if proj is not None:
            cf = ax_map.pcolormesh(x, y, sslice, cmap=pal, norm=pal_norm, transform=ccrs.PlateCarree())
        else:
    ...
    if contour:
        if not fill:
            ...
            yc = y
        if proj is not None:
            ax_map.contour(xc, yc, sslice, vc[::d], colors='k', linewidths=0.5, linestyles='solid', transform=ccrs.PlateCarree())
        else:
